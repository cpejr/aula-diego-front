...
function LinhaTabelaTurmas(props) {
  ...
  function removeLine(matricula) {
    ...
    const filtrado = copy.filter((item) => {
      return matricula != item.matricula;
    });
    ...
    <div className="LinhaTabelaTurmas">
      ...
      <div style={{ display: "flex", alignItems: "center", maxWidth: "100%" }}>
        ...
        <HighlightOffIcon
          ...
          onClick={() => props.excluirLinha(props.item.matricula)}
        />
      </div>
    </div>
  );
    ...
    <div>
      <div className="TabelaTurmasBox">
        <div className="LinhaBoard">
          <p className="TabelaTurmasLabel" style={{ borderBottom: "none" }}>
            Nome
          </p>
          <p className="TabelaTurmasLabel" style={{ borderBottom: "none" }}>
            Matrícula
          ...
          <p
            className="TabelaTurmasLabel"
            style={{ borderRight: "none", borderBottom: "none" }}
          >
            Ocupação
          </p>
        </div>
        {filtrar(tempData).map((item) => {
          return (
            <LinhaTabelaTurmas
              key={item.matricula}
              item={item}
